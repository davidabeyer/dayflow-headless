...
struct SettingsView: View {
    ...
    @State private var geminiPromptState = PromptCustomizationState<GeminiPromptOverrides>(
        userDefaultsKey: GeminiPromptConfig.userDefaultsKey
    )
    @State private var geminiFields: [String: PromptFieldState] = GeminiPromptConfig.defaultFields()

    // Ollama prompt customization
    @State private var ollamaPromptState = PromptCustomizationState<OllamaPromptOverrides>(
        userDefaultsKey: OllamaPromptConfig.userDefaultsKey
    )
    @State private var ollamaFields: [String: PromptFieldState] = OllamaPromptConfig.defaultFields()
    ...
    var body: some View {
        ...
        .onChange(of: geminiFields) { _ in
            geminiPromptState.persist(GeminiPromptOverrides.self, createOverrides: GeminiPromptConfig.createOverrides)
        }
        .onChange(of: ollamaFields) { _ in
            ollamaPromptState.persist(OllamaPromptOverrides.self, createOverrides: OllamaPromptConfig.createOverrides)
        }
    }
        ...
        VStack(alignment: .leading, spacing: 28) {
            ...
            if currentProvider == "gemini" {
                ...
                SettingsCard(title: "Gemini prompt customization", subtitle: "Override Dayflow's defaults to tailor card generation") {
                    PromptCustomizationView(
                        introText: "Overrides apply only when their toggle is on. Unchecked sections fall back to Dayflow's defaults.",
                        fields: GeminiPromptConfig.fields,
                        fieldStates: $geminiFields,
                        onReset: resetGeminiPrompts
                    )
                }
            } else if currentProvider == "ollama" {
                SettingsCard(title: "Local prompt customization", subtitle: "Adjust the prompts used for local timeline summaries") {
                    PromptCustomizationView(
                        introText: "Customize the local model prompts for summary and title generation.",
                        fields: OllamaPromptConfig.fields,
                        fieldStates: $ollamaFields,
                        onReset: resetOllamaPrompts
                    )
                }
            }
    ...
    private func loadPromptCustomizations() {
        if currentProvider == "gemini" {
            geminiPromptState.load(GeminiPromptOverrides.self, extractFields: GeminiPromptConfig.extractFields)
            geminiFields = geminiPromptState.fields.isEmpty ? GeminiPromptConfig.defaultFields() : geminiPromptState.fields
        } else if currentProvider == "ollama" {
            ollamaPromptState.load(OllamaPromptOverrides.self, extractFields: OllamaPromptConfig.extractFields)
            ollamaFields = ollamaPromptState.fields.isEmpty ? OllamaPromptConfig.defaultFields() : ollamaPromptState.fields
        }
    }

    private func resetGeminiPrompts() {
        geminiPromptState.reset(defaultFields: GeminiPromptConfig.defaultFields())
        geminiFields = GeminiPromptConfig.defaultFields()
    }

    private func resetOllamaPrompts() {
        ollamaPromptState.reset(defaultFields: OllamaPromptConfig.defaultFields())
        ollamaFields = OllamaPromptConfig.defaultFields()
    }
