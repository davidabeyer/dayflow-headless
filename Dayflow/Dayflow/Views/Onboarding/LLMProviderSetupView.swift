...
class ProviderSetupState: ObservableObject {
    ...
    // Generic CLI stream runner to avoid duplication
    private func runStream(
        ...
        guard let path = CLIDetector.resolveExecutablePath(for: tool) else {
            output = "\(tool.displayName) not found."
            return
        }
        ...
        streamer.run(
            ...
            onStdout: { [weak self] chunk in
                self?.appendToOutput(chunk, for: tool)
            },
            onStderr: { [weak self] chunk in
                self?.appendToOutput("\n[stderr] \(chunk)", for: tool)
            },
            onFinish: { [weak self] code in
                self?.finishStream(exitCode: code, for: tool)
            }
        )
    }

    private func appendToOutput(_ text: String, for tool: CLITool) {
        switch tool {
        case .codex: codexStreamOutput.append(text)
        case .claude: claudeStreamOutput.append(text)
        }
    }

    private func finishStream(exitCode: Int32, for tool: CLITool) {
        switch tool {
        case .codex:
    ...
    func runCodexStream() {
        runStream(for: .codex, args: ["exec", "--json", cliPrompt], streamer: codexStreamer, output: &codexStreamOutput, isRunning: &isRunningCodexStream)
    }
    ...
    func runClaudeStream() {
        runStream(for: .claude, args: ["--print", "--output-format", "json", cliPrompt], streamer: claudeStreamer, output: &claudeStreamOutput, isRunning: &isRunningClaudeStream)
    }
