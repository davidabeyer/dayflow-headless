    ...
    func generateActivityCards(observations: [Observation], context: ActivityGenerationContext, batchId: Int64?) async throws -> (cards: [ActivityCardData], log: LLMCall) {
        ...
        while attempt < maxRetries {
            do {
                ...
                // Validation phase
                let (coverageValid, coverageError) = ActivityCardValidator.validateTimeCoverage(existingCards: context.existingCards, newCards: normalizedCards)
                let (durationValid, durationError) = ActivityCardValidator.validateTimeline(normalizedCards)
