    ...
    func processBatch(_ batchId: Int64, completion: @escaping (Result<ProcessedBatchResult, Error>) -> Void) {
        Task {
            ...
            do {
                ...
                // IDLE DETECTION: Skip Gemini API call for static screens
                if IdleDetector.isIdlePeriod(videoSizeBytes: videoData.count, durationSeconds: totalDuration) {
                    let bpm = IdleDetector.bytesPerMinute(videoSizeBytes: videoData.count, durationSeconds: totalDuration)
                    print("💤 [LLMService] Detected idle period - \(videoData.count / 1024) KB for \(Int(totalDuration))s (\(Int(bpm / 1024)) KB/min)")

                    let idleObservation = Observation(
                        id: nil,
                        batchId: batchId,
                        startTs: batchStartTs,
                        endTs: batchEndTs,
                        observation: "IDLE - Screen unchanged for \(Int(totalDuration / 60)) minutes. No user activity detected.",
                        metadata: nil,
                        llmModel: nil,
                        createdAt: Date()
                    )
                    ...
                    StorageManager.shared.saveObservations(batchId: batchId, observations: [idleObservation])
                    StorageManager.shared.updateBatch(batchId, status: "analyzed")

                    await AnalyticsService.shared.capture("idle_period_detected", [
                        "batch_id": batchId,
                        "duration_seconds": Int(totalDuration),
                        "video_size_kb": videoData.count / 1024
                    ])
